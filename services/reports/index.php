<?php
	...
	function path_clean ($PATH="") {
		...
		#echo "$REPOSITORY/$GROUP/$PROJECT/$RUN/$SAMPLE/ <br>";
		return "$REPOSITORY/$GROUP/$PROJECT/$RUN/$SAMPLE/";

	};

	#echo "TEST clean: ".path_clean ("*/*/*/");
	...
	$PATH_SPLIT=explode ( "/" , $PATH );
	$REPOSITORY=$PATH_SPLIT[0];
	$GROUP=$PATH_SPLIT[1];
	$PROJECT=$PATH_SPLIT[2];
	$RUN=$PATH_SPLIT[3];
	$SAMPLE=$PATH_SPLIT[4];

	$PATH_HTML="<a href='?PATH=*/*/*/*/*/'>HOME</a>"; #.str_replace("/"," > ",str_replace("*/","",$PATH));
	...
	function links_from_path ($HOME="repository",$PATH="") {
		foreach (glob ( $HOME."/".$PATH."/*", GLOB_ONLYDIR ) as $key => $value) {
			#$LINKS_HTML.="<a href='?PATH=$PATH".end(explode( "/", $value ))."/*/*/*/*/'>".end(explode( "/", $value ))."</a> ";
			$LINKS_HTML.="<a href='?PATH=".path_clean("$PATH".end(explode( "/", $value )))."'>".end(explode( "/", $value ))."</a> ";
		};
		echo "L $LINKS_HTML L";
		return $LINKS_HTML;
	};
	...
	if ($REPOSITORY=="*") {
		$LINKS_HTML=links_from_path ("$HOME","");
	} else {
		$PATH_HTML.=" > <a href='?PATH=$REPOSITORY/*/*/*/*/'>$REPOSITORY</a>";
		if ($GROUP=="*") {
			$LINKS_HTML=links_from_path ("$HOME","$REPOSITORY/");
		} else {
			$PATH_HTML.=" > <a href='?PATH=$REPOSITORY/$GROUP/*/*/*/'>$GROUP</a>";
			if ($PROJECT=="*") {
				$LINKS_HTML=links_from_path ("$HOME","$REPOSITORY/$GROUP/");
			} else {
				$PATH_HTML.=" > <a href='?PATH=$REPOSITORY/$GROUP/$PROJECT/*/*/'>$PROJECT</a>";
				if ($RUN=="*") {
					$LINKS_HTML=links_from_path ("$HOME","$REPOSITORY/$GROUP/$PROJECT/");
				} else {
					$PATH_HTML.=" > <a href='?PATH=$REPOSITORY/$GROUP/$PROJECT/$RUN/*/'>$RUN</a>";
					if ($SAMPLE=="*") {
						$LINKS_HTML=links_from_path ("$HOME","$REPOSITORY/$GROUP/$PROJECT/$RUN/");
					} else {
						$PATH_HTML.=" > <a href='?PATH=$REPOSITORY/$GROUP/$PROJECT/$RUN/$SAMPLE/'>$SAMPLE</a>"; #.str_replace("/"," > ",str_replace("*/","",$PATH));
					};
				};
			};
		};
	...
	echo '
		<section class="header1 cid-ru7OEConn1" id="header16-1k">
		    ...
		    <div class="container">
				...
				<h3 class="mbr-section-subtitle mbr-fonts-style align-center mbr-light display-7">
					'.$PATH_HTML.'
					<br><br>
					'.$LINKS_HTML.'
			  	</h3>
	...
	echo "<br>PATH=$PATH";
	echo "<br>PATH=repositories/$PATH*stark.report.html";
	$tbody="";
	#$reports=glob ( "repositories/*/*/*/*/*/*stark.report.html" );
	#$reports=glob ( "repositories/$REPOSITORY/*/*/*/*/*stark.report.html" );
	$reports=glob ( "repositories/$PATH*stark.report.html" );
