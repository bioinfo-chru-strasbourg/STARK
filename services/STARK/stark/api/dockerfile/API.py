...
def stark_launch():
	...
	if 'run' in json_input.keys():
		...
		if len(json_input['run'].split(",")) == 1:
			...
			runMD5 = subprocess.run(myCmd, shell=True, stdout=subprocess.PIPE).stdout.decode('utf-8').strip()
	...
	#print(myCmd);
	getCmd = subprocess.run(myCmd, shell=True, stdout=subprocess.PIPE).stdout.decode('utf-8')
	...
	if ts != "":
		...
		getPostCmd = subprocess.run(myPostCmd, shell=True, stdout=subprocess.PIPE).stdout.decode('utf-8').strip()
...
def queue():
	...
	getCmd = subprocess.run(myCmd, shell=True, stdout=subprocess.PIPE).stdout.decode('utf-8')
	if getCmd and not isinstance(getCmd, bytes):
		ret_val=200
	else:
	...
	# output
	if request.args.get('format'):   #format  == "html":
		ret='<pre>'+home_header+'\n'+header+getCmd+'</pre>'
	else:
		ret=getCmd
