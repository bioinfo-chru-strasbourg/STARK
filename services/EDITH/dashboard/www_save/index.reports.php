...
if ($INPUT_SAMPLE!="*" || $INPUT_ANALYSIS!="*") {
	$PATH="repositories/*/*/*/".$INPUT_ANALYSIS."/".$INPUT_SAMPLE;
}
...
if (count($reports)<=$MAX_REPORTS) {
	...
	foreach ($reports as $key => $report_html) {
		...
		foreach ($plugins_obj as $plugin=>$plugin_infos) {
			#print_r($plugins_obj);
			#print "<br>";
			#print is_file($plugins_folder."/".$plugin_infos->{"script"})."<br>";
			#print $modules_obj_array[$plugin_infos->{"module"}]->{"available"}."<br>";
			if (is_file($plugins_folder."/".$plugin_infos->{"script"})				# script exists
				&& $plugin_infos->{"available"}				# script exists
				...
				if ($uri!="") {
					foreach ($plugin_infos->{"level"} as $level=>$level_infos) {
						...
						#$link="<a href='$url_full'>".$plugin_infos->{"label"}."</a>";
						$PLUGINS_LINKS[$level].=" ".$link." ";
					};
					#print "URI=$uri<br>";
				};
				#print_r($PLUGINS_LINKS);
				#print "<br>";
			}
			
		}

		// if ($plugins_obj->{"IGV"}->{"level"}->{"sample"} && is_file($plugins_folder."/".$plugins_obj->{"IGV"}->{"script"})) {
		// #if ($plugins_obj->{"IGV"}->{"level"}->{"sample"}) {
		// 	echo "SAMPLE<br>".$plugins_folder."/".$plugins_obj->{"IGV"}->{"script"}."<br>";
		...
		// #print_r($plugins_obj->{"IGV"});
		// if (isset($MODULES["IGV"]) || 1) {
