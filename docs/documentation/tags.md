...
Tags format is basically "TYPE#TAG". But the formal pattern is:
...
Explanation:
- TYPE can be null (e.g. "#TAG").
- TAG null will not be considered (e.g. "TYPE#").
- TAG are separated by "!" (e.g. "TYPE1#TAG1!TYPE2#TAG").
- Words without "#" will not be considered as TAG (e.g. "information not a tag").
- TAG can be cumulative for a TYPE (e.g. "TYPE#TAG1#TAG2", same as "TYPE#TAG1!TYPE#TAG2").
...
This example allow to launch run analysis with EXOME application. Moreover, the run is tagged with custom "#ONCOLOGY MTP-Nano500V2" tag.

Concerning samples:

- "Sample1" is tagged as "female" ("SEX#F") and customly associated to lymphoma pathology ("PATHOLOGY#lymphoma")
- "Sample2" is tagged as "male" ("SEX#M") and customly associated to lung (#LUNG), to cancer (#CANCER) and to a metastatic relapse ("#RELAPSE")
- "control" is tagged as a sample specifically mentioned as an internal control ("SAMPLE_TYPE#internal_control")

## Where to find tags in STARK analysis results
...
By searching tags within ".tag" files, specific triggers can launch external services to complement main STARK analysis (e.g. "CNV#CANOE" to launch CNV external analysis with CANOE tool if associated STARK module/service is activated)
