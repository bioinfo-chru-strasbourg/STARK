...
function usage {
	...
	echo "# -s/--sample=<STRING1,STRING2,...>           Samples to use for the analysis";
...
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# ":" tells that the option has a required argument, "::" tells that the option has an optional argument, no ":" tells no argument
ARGS=$(getopt -o "e:r:a:c:k:p:f:s:l:zum:wyvdnh" --long "env:,app:,application:,runs:,aligners:,callers:,annotators:,pipelines:,filter_samples:,sample:,samplesheet:,parallelization,by_sample,remove:,application,pipelines_infos,verbose,debug,release,help" -- "$@" 2> /dev/null)
[ $? -ne 0 ] && \
		...
		if [ -d "$MISEQ_FOLDER/$RUN" ] ; then
			...
			if [[ $(echo $SAMPLES_PROJECT_LIST | wc -w) -gt 1 ]]; then
				...
				do
					...
					RUN_APP_RELEASE=$($SCRIPT_DIR/extract_variable_from_env.sh $RUN_ENV "APP_RELEASE")
					RUN_APP_FROM_FILE=$(echo $(basename $RUN_ENV) | sed "s/^env.//gi" | sed "s/.sh$//gi" | sed "s/sh$//gi")
					if [ "$RUN_APP" == "" ]; then RUN_APP=$RUN_APP_NAME; fi
					if [ "$RUN_APP" == "" ]; then RUN_APP=$RUN_APP_FROM_FILE; fi
					...
					echo "# APP RELEASE:           $RUN_APP_RELEASE"
