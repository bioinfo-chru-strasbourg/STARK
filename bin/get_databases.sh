...
if [ ! -e $REF ]; then
	...
	for CHROM_FILE in $CHRS; do
	    ...
	    echo "$TMP_DATABASES_DOWNLOAD_FOLDER/$ASSEMBLY/chromosomes/$CHROM_FILE:
			wget --no-verbose -S -c -O $TMP_DATABASES_DOWNLOAD_FOLDER/$ASSEMBLY/chromosomes/$CHROM_FILE $UCSC_GENOME_URL/$CHROM_FILE
	    " >> $MK
...
if [ ! -e $REFSEQ_GENES ]; then
	...
	echo "$TMP_DATABASES_DOWNLOAD_FOLDER/$ASSEMBLY/$REFSEQ_GENES_TXT_FILE: $DBFOLDER
		...
		wget --no-verbose -S -c -O $TMP_DATABASES_DOWNLOAD_FOLDER/$ASSEMBLY/refGene.txt.gz $REFSEQ_GENES_URL/refGene.txt.gz ;
	...
	MK_ALL="$MK_ALL $REFSEQ_GENES $REFSEQ_GENES_TXT $REFSEQ_GENES_GENES"
...
if [ ! -e $VCFDBSNP ] || (($UPDATE)); then
	...
	echo "$VCFDBSNP: $DBFOLDER
		...
		wget --no-verbose -S -c -O $TMP_DATABASES_DOWNLOAD_FOLDER/$VCFDBSNP_FOLDER/$DBSNP_LATEST.tbi $DBSNP_LATEST_FILE.tbi;
		wget --no-verbose -S -c -O $TMP_DATABASES_DOWNLOAD_FOLDER/$VCFDBSNP_FOLDER/$DBSNP_LATEST $DBSNP_LATEST_FILE;
