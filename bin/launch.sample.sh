		...
		if ((1)); then
			...
			if [ "$RESULTS_FOLDER_COPY_ALL" != "$RESULTS" ] && [ "$RESULTS_FOLDER_COPY_ALL" != "" ] ; then
				...
				do
					...
					$COMMAND_COPY $RESULTS/$RUU/$S/* $RESULTS_FOLDER_COPY_FOLDER/$RUU/$S/$RESULTS_SUBFOLDER_DATA 1>>$LOGFILE_RES_RUN 2>>$LOGFILE_RES_RUN
					# Copy RUN files
					$COMMAND_COPY $(find -L $RESULTS/$RUU -mindepth 1 -maxdepth 1 -type f) $RESULTS_FOLDER_COPY_FOLDER/$RUU 1>>$LOGFILE_RES_RUN 2>>$LOGFILE_RES_RUN
						...
						for ROOT_FILE_PATTERN_INFO in $REPOSITORY_FILE_PATTERNS; do
							...
							for F_SOURCE in $(ls $FILE_PATTERN_TYPE_LS $RESULTS_FOLDER_COPY_FOLDER/$RUU/$S/$RESULTS_SUBFOLDER_DATA$ROOT_FILE_PATTERN_VAR 2>/dev/null); do
								...
								if [ ! -e $F_TARGET ]; then
									...
									$COMMAND_COPY $F_SOURCE $F_TARGET 1>>$LOGFILE_RES_RUN 2>>$LOGFILE_RES_RUN
								fi;
								if [ ! -e $F_TARGET ]; then
