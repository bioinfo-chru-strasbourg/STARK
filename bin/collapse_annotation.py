...
class Transcript:
    def __init__(
        self, transcript_id, transcript_name, transcript_type, gene, start_pos, end_pos
    ):
...
class Gene:
    def __init__(
        self, gene_id, gene_name, gene_type, chrom, strand, start_pos, end_pos
    ):
        self.id = gene_id
        self.name = gene_name
    ...
    def __init__(self, gtfpath):
        ...
        with opener as gtf:
            for row in gtf:
                ...
                for a in (
                    row[8].replace('"', "").replace("_biotype", "_type").split(";")[:-1]
                ):
                    kv = a.strip().split(" ")
                    if (
                        len(kv) == 2
                    ):  # ignore attributes without values (e.g. transcript_id "")
                        if kv[0] != "tag":
...
def collapse_annotation(
    ...
    for g in annot.genes:
        ...
        for t in g.transcripts:
            if (
                (t.id not in blacklist)
                and (t.type != "retained_intron")
                and (
                    ("tags" not in t.attributes)
                    or len(set(t.attributes["tags"]).intersection(exclude)) == 0
                )
            ):
                for e in t.exons:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Collapse isoforms into single transcript per gene and remove overlapping intervals between genes"
    )
    ...
    parser.add_argument(
        ...
        help="List of transcripts to exclude (e.g., unannotated readthroughs)",
    )
    parser.add_argument(
        "--collapse_only",
        action="store_true",
        help="Only collapse transcripts of each gene, do not remove overlaps.",
    )
    parser.add_argument(
        "--stranded",
        action="store_true",
        help="Only consider genes on the same strand when removing overlaps.",
    )
