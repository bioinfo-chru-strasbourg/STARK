...
LOGO="$STARK_FOLDER_DOCS/logo/logo_STARK.png"


[ ! -e "$LOGO" ] && LOGO="$STARK/logo_report.jpg"
...
if [ -z $OUTPUT_INPUT ]; then
	...
	OUTPUT_HTMLPDFFILE=$RESULTS_FOLDER/$FLOWCELL/$SAMPLE/$SAMPLE.reports/$SAMPLE.$DATE.stark.html.report.pdf
else
	OUTPUT_PDFFILE=$OUTPUT_INPUT
	OUTPUT_HTMLPDFFILE=$OUTPUT_INPUT.html.pdf
fi;
...
HTMLFILE="$REPORTDIR/$SAMPLE.$DATE.stark.html.report.html"
HTMLPDFFILE="$REPORTDIR/$SAMPLE.$DATE.stark.html.report.pdf"
HTMLFOLDER="$SAMPLE.$DATE.stark.html.report.html.folder"
mkdir -p $REPORTDIR/$HTMLFOLDER
#PDFFILE="$REPORTDIR/$SAMPLE.$DATE.stark.report.pdf"
...
<style>
   body{
	...
	.tableau {
		counter-increment: tableaux;
		padding: 5px;
	}

	.tableau tableaucaption:before {
		content: "Table " counter(h2counter) "." counter(tableaux) " - ";
		padding: 5px;
	}
...
echo -e "
<TABLE>
<TR><TD>Sample:</TD><TD>$SAMPLEID</TD></TR>
<TR><TD>Analysis:</TD><TD>$FLOWCELL</TD></TR>
<TR><TD>Group:</TD><TD>$GROUP</TD></TR>
<TR><TD>Project:</TD><TD>$PROJECT</TD></TR>
<TR><TD>Assembly:</TD><TD>$ASSEMBLY</TD></TR>
<TR><TD>STARK:</TD><TD>$STARK_VERSION</TD></TR>
<TR><TD>Application:</TD><TD>$APP_NAME</TD></TR>
<TR><TD>Design:</TD><TD>$MANIFEST_NAME"$(if [ "$MANIFEST_NAME" != "$BED_NAME" ]; then echo " / $BED_NAME"; fi)"</TD></TR>
<TR><TD>#TAG Sample:</TD><TD>"$(cat $RESULTS_FOLDER/$FLOWCELL/$SAMPLE/$SAMPLE.tag | tr "!" " ")"</TD></TR>
<TR><TD>#TAG Analysis:</TD><TD>"$(cat $RESULTS_FOLDER/$FLOWCELL/$SAMPLE/$SAMPLE.analysis.tag | tr "!" " ")"</TD></TR>
</TABLE>
" > $HTMLFILE.main
...
# sequencing
cp $FASTQCIMAGESDIR/per_base_quality.png $REPORTDIR/$HTMLFOLDER/per_base_quality.png
cp $FASTQCIMAGESDIR/per_base_n_content.png $REPORTDIR/$HTMLFOLDER/per_base_n_content.png
cp $FASTQCIMAGESDIR/per_base_sequence_content.png $REPORTDIR/$HTMLFOLDER/per_base_sequence_content.png
cp $FASTQCIMAGESDIR/duplication_levels.png $REPORTDIR/$HTMLFOLDER/duplication_levels.png
cp $FASTQCIMAGESDIR/adapter_content.png $REPORTDIR/$HTMLFOLDER/adapter_content.png
cp $LOGO $REPORTDIR/$HTMLFOLDER/logo
...
<article>
<p>
FastQC performs analyses to assess the quality of the data.
</p>

<div class='tableau'>
	<TABLE border=1>
		<TR><TD>Total number of reads</TD><TD>$TotalSequences</TD></TR>
		<TR><TD>Unique number of reads</TD><TD>$UniqueNumberReads</TD></TR>
		<TR><TD>% of unique number reads</TD><TD>$PercentUniqueNumberReads</TD></TR>
		<TR><TD>Sequence length</TD><TD>$SequenceLength</TD></TR>
		<TR><TD>Q30</TD><TD>$Q30_print</TD></TR>
	</TABLE>
	<tableaucaption>Sequencing information.</tableaucaption>
</div>
<br>
<center>
	<IMG src='$HTMLFOLDER/per_base_quality.png' width='$IMG_WIDTH'><BR>
	<!--<IMG src='$HTMLFOLDER/per_base_n_content.png' width='$IMG_WIDTH'><BR>-->
	<IMG src='$HTMLFOLDER/per_base_sequence_content.png' width='$IMG_WIDTH'><BR>
	<IMG src='$HTMLFOLDER/duplication_levels.png' width='$IMG_WIDTH'><BR>
	<!--<IMG src='$HTMLFOLDER/adapter_content.png' width='$IMG_WIDTH'><BR>-->
</center>
</article>
...
do
	...
	#if [ -f "$COVERAGEFILESTATS_STATFILE" ]; then
		Total_qc_passed_reads=$(grep "in total" $COVERAGEFILESTATS_STATFILE | cut -d"+" -f1 | sed 's/ //')
		Percent_qc_passed_reads=$(echo "scale=2;(($Total_qc_passed_reads/$TotalSequences)*100)/1" | bc)
		Total_mapped_reads=$(grep "mapped (" $COVERAGEFILESTATS_STATFILE | cut -d"+" -f1 | sed 's/ //')
		Percent_mapped_reads=$(grep "mapped (" $COVERAGEFILESTATS_STATFILE | cut -d"(" -f2 | sed 's/[%:].*//')
		READS_ON_TARGET=$(echo $(cat $COVERAGEFILESTATS_ONREADS_SAMTOOLS)" +0 " | bc);
		READS_OFF_TARGET=$(echo $(cat $COVERAGEFILESTATS_OFFREADS_SAMTOOLS)" +0 " | bc);
		READS_ALL_TARGET=$(echo "$READS_ON_TARGET+$READS_OFF_TARGET" | bc)
		READS_ON_TARGET_PERCENTAGE=$(echo "scale=4;(($READS_ON_TARGET/$READS_ALL_TARGET)*100)/1" | bc)
		READS_OFF_TARGET_PERCENTAGE=$(echo "scale=4;(($READS_OFF_TARGET/$READS_ALL_TARGET)*100)/1" | bc)

		echo "
		<div class='tableau'>
		<table border='1' class='.table_horizontal'>
			<tr><th></th><th>Number of Reads</th><th>Percentage of Reads</th></tr>
			<tr><td>QC-passed</td><td>$Total_qc_passed_reads</td><td></td></tr>
			<tr><td>Mapped</td><td>$Total_mapped_reads</td><td>$Percent_mapped_reads%</td></tr>
			<tr><td>ON Target</td><td>$READS_ON_TARGET</td><td>$READS_ON_TARGET_PERCENTAGE%</td></tr>
			<tr><td>OFF Target</td><td>$READS_OFF_TARGET</td><td>$READS_OFF_TARGET_PERCENTAGE%</td></tr>
		</table>
		<tableaucaption>
			Reads Alignment statistics with $aligner. Statistics were determined with Samtools flagstat and Bedtools on the $SAMPLEID.$aligner.bam file (duplicates and bad quality reads removed to calculate ON/OFF tagets).
		</tableaucaption>
		</div>
		" >> $HTMLFILE.mapping
	#fi;
	...
	if [ -f "$COVERAGEFILESTATS_SAMTOOLS" ]; then
		...
		echo "
		<p>
			Sequencing coverage estimation on target ( total number of sequenced bases / number bases of target) : "$(echo "scale=2;($COV_CALC)/1" | bc)" X
		</p>" >> $HTMLFILE.coverage
	fi
	...
	if [ -f "$COVERAGEFILESTATS_MARKDUPLICATESFILE" ]; then
		...
		<div class='tableau'>
			<table border=1>
				<tr><td>Read pairs examined</td><td>$READ_PAIRS_EXAMINE</td></tr>
				<tr><td>Percent duplication</td><td>"$(echo "scale=2;($PERCENT_DUPLICATION * 100)/1" | bc)"</td></tr>
			</table>
			<tableaucaption>
				Markduplicate statistics on $aligner on $SAMPLEID. Statistics were determined with GATK Markduplicates on the $SAMPLEID.$aligner.bam file. See file '"$(basename $COVERAGEFILESTATS_MARKDUPLICATESFILE)"' for more information.
			</tableaucaption>
		</div>" >> $HTMLFILE.target_coverage
	fi;

	# COVERAGE SUMMARY ON/OFF target with HsMetrics
	if ((0)) && [ -f "$COVERAGEFILESTATS_HSMETRICS" ]; then
		...
		if [ "$PCT_SELECTED_BASES" != "" ] && [ "$PCT_OFF_BAIT" != "" ]; then
			...
			<div class='tableau'>
				<table border=1>
					<tr><td>% ON target</td><td>"$(echo "scale=2;($PCT_SELECTED_BASES*100)/1" | bc)"</td></tr>
					<tr><td>% OFF target</td><td>"$(echo "scale=2;($PCT_OFF_BAIT*100)/1" | bc)"</td></tr>
				</table>
				<tableaucaption>
					Coverage statistics between on and off targets reads on $aligner on $SAMPLEID. Statistics were determined with Picard HSMetrics on the $SAMPLEID.$aligner.bam file. See file '"$(basename $COVERAGEFILESTATS_HSMETRICS)"' for more information.
				</tableaucaption>
			</div>" >> $HTMLFILE.target_coverage
	...
	then
		...
		cat $COVERAGEFILESTATS_SAMTOOLS | awk -v MDP=$MDP '{SUM++} { if ($3>MDP) {DP[MDP]++} else {DP[$3]++} } END { for (i=MDP; i>=0; i-=1) {print i" "DP[i]" SUM"SUM}}' | sort -g -r | awk -v COVS=$COVS '{SUM+=$2} {CUM[$1]=SUM} {split(COVS,C,",")}  END  { for (j in C) {print C[j]" "CUM[C[j]]" SUM "(CUM[C[j]]/SUM)} }' | sort -g | awk '{print "<tr><td>"$1"X</td><td>"($4*100)"%</td></tr>"}' > $COVERAGEFILESTATS_SAMTOOLS.summary.tmp.html

		echo "
		<div class='tableau'>
		<table border='1' class='.table_horizontal'>
			<tr><th>Coverage</th><th>%targeted bases</th></tr>
			"$(cat $COVERAGEFILESTATS_SAMTOOLS.summary.tmp.html)"
		</table>
		<tableaucaption>
			Coverage statistics on targeted regions on $aligner on $SAMPLEID. Statistics were determined with SAMTOOLS (duplicates and bad quality reads removed) on the $SAMPLEID.$aligner.bam file. See file '"$(basename $COVERAGEFILESTATS_SAMTOOLS)"' for more information.
		</tableaucaption>
		</div>
		" >> $HTMLFILE.target_coverage

		rm -f $COVERAGEFILESTATS_SAMTOOLS.summary.tmp.html;
	...
	do
		...
		then
			...
			<div class='tableau'>
			<table border='1' class='.table_horizontal'>
				<tr><th>Threshold</th><th>Number of Genes</th><th>Genes</th></tr>
				<tr><td>PASS</td><td>$(awk -F"\t" '$2=="PASS" {print $1}' $COVERAGEFILESTATS_GENES_COVERAGE_MSG | wc -w) genes</td><td>$((($(awk -F"\t" '$2=="PASS" {print $1}' $COVERAGEFILESTATS_GENES_COVERAGE_MSG | wc -w))) && echo " "$(awk -F"\t" '$2=="PASS" {print $1}' $COVERAGEFILESTATS_GENES_COVERAGE_MSG | tr "\n" " " | cut -f 1-$MAX_GENES -d " ")) $([ "$(awk -F"\t" '$2=="PASS" {print $1}' $COVERAGEFILESTATS_GENES_COVERAGE_MSG | wc -w)" -gt $MAX_GENES ] && echo "... (only the first $MAX_GENES)")</td></tr>
				<tr><td>WARN</td><td>$(awk -F"\t" '$2=="WARN" {print $1}' $COVERAGEFILESTATS_GENES_COVERAGE_MSG | wc -w) genes</td><td class='warn'>$((($(awk -F"\t" '$2=="WARN" {print $1}' $COVERAGEFILESTATS_GENES_COVERAGE_MSG | wc -w))) && echo " "$(awk -F"\t" '$2=="WARN" {print $1}' $COVERAGEFILESTATS_GENES_COVERAGE_MSG | tr "\n" " " | cut -f 1-$MAX_GENES -d " ")"") $([ "$(awk -F"\t" '$2=="WARN" {print $1}' $COVERAGEFILESTATS_GENES_COVERAGE_MSG | wc -w)" -gt $MAX_GENES ] && echo "... (only the first $MAX_GENES)")</td></tr>
				<tr><td>FAIL</td><td>$(awk -F"\t" '$2=="FAIL" {print $1}' $COVERAGEFILESTATS_GENES_COVERAGE_MSG | wc -w) genes</td><td class='fail'>$((($(awk -F"\t" '$2=="FAIL" {print $1}' $COVERAGEFILESTATS_GENES_COVERAGE_MSG | wc -w))) && echo " "$(awk -F"\t" '$2=="FAIL" {print $1}' $COVERAGEFILESTATS_GENES_COVERAGE_MSG | tr "\n" " " | cut -f 1-$MAX_GENES -d " ")"") $([ "$(awk -F"\t" '$2=="FAIL" {print $1}' $COVERAGEFILESTATS_GENES_COVERAGE_MSG | wc -w)" -gt $MAX_GENES ] && echo "... (only the first $MAX_GENES)")</td></tr>
			</table>
			<tableaucaption>
				$GENES_COVERAGE_MSG_LEGEND
			</tableaucaption>
			</div>
			" >> $HTMLFILE.gene_coverage
...
echo "
	<div class='tableau'>
	<table border='1' class='.table_horizontal'>
		<tr><th>Pipeline</th><th>Number of variant</th></tr>
		" >> $HTMLFILE.variant_calling
...
do
	...
	do
		...
		do
			...
			then
				VCFFILE=$RESULTS_FOLDER/$FLOWCELL/$SAMPLE/$SAMPLE.$aligner.$caller.$annotator.vcf
				number_variants="?" && [ -e $VCFFILE ] && number_variants=$(grep -cv "^#" $VCFFILE)

				echo "
				<tr><td>"$(echo $aligner.$caller.$annotator)"</td><td>$number_variants</tr>
				" >> $HTMLFILE.variant_calling
...
<tableaucaption>
	Number of variants by pipeline (Total number of variants: $number_variants_final).
</tableaucaption>
</div>
" >> $HTMLFILE.variant_calling
...
for PZFLAG_FIELD in $(grep "^#CHROM" $FINALTXTFILE | tr "\t" "\n" | grep "^PZFlag"); do
	...
	if (($NB_PASS)); then
		...
		<div class='tableau'>
		<table border='1' class='.table_horizontal'>
			<tr><th>HGVS</th><th>Location Outcome Impact</th><th>VAF</th></tr>
			"$(head -n$(echo "$NB_VARIANT_LIMIT +1" | bc) $FINALTXTFILE | awk -f $SCRIPT_DIR/tsv_extract.awk -F"\t" -v COLS="NOMEN,location,outcome,snpeff_impact,VAF_median,$PZFLAG_FIELD,#CHROM,POS,REF,ALT" -v DEFAULT_NA="" | sed 's/&/ /gi' | awk -F "\t" '{split($1,NOMEN,",")} {CHROMPOSREFALT=$7":"$8$9">"$10} length(NOMEN[1])==0 {NOMEN[1]=CHROMPOSREFALT} length(NOMEN[1])<=50 {HGVS=NOMEN[1]} {HGVS=NOMEN[1]} length(NOMEN[1])>50 {HGVS=substr(NOMEN[1],1,47)"..." } {OUTPUT=$2} {LOCATION=$3} {IMPACT=$4} {VAF=$5*100} {PZFlag=$6} PZFlag == "PASS" {print "<tr><td>"HGVS"</td><td>"LOCATION" "OUTPUT" "IMPACT"</td><td>"VAF"%</td></tr>"}')"

		</table>
		<tableaucaption>
			$NB_PASS Variants passing prioritization filter '$HOWARD_PRIORITIZATION_REPORT_NAME' (first $NB_VARIANT_LIMIT variants).
		</tableaucaption>
		</div>
		" >> $HTMLFILE.variant_annotation_prioritization
...
echo "COVERAGEFILESTATS_HSMETRICS"$COVERAGEFILESTATS_HSMETRICS
		...
		if ((1)); then
			...
			"$(
				...
				END {
					for (rowNr=1;rowNr<=maxRows;rowNr++) {
						for (colNr=1;colNr<=maxCols;colNr++) {
							printf "%s %s", " "cell[rowNr,colNr], (colNr < maxCols ? "\t" : "\n")
						}
					}
			...
			<tableaucaption>
				Coverage statistics on each targeted region (primers excluded if any) on $aligner on $SAMPLEID. Statistics were determined with PICARD on the $SAMPLEID.$aligner.bam file.
			</tableaucaption>
			</div>
			" >> $HTMLFILE.annexe
		fi;
...
<body>
	...
	<header>
		<div>
			<table width="100%"  border="0">
				<tr>
					<td width="*" align="left"><img height="150" src="'$HTMLFOLDER'/logo"></td>
					<td width="*" align="center"><h1>'$SAMPLEID'<br></h1><h2 class="nocount">'$FLOWCELL'</h2></td>
					<td width="*" align="right"><h2 class="nocount">'$GROUP'<br></h2><h2 class="nocount">'$PROJECT'</h2><br></td>
				</tr>
			<table>
		</div>
		<nav>
			<h2 class="nocount">Content</h2>
			<ul>
			...
			<li>
				<ul>
					...
					<li><a href="#target_coverage">Target Coverage</a></li>
					<li><a href="#gene_coverage">Gene Coverage</a></li>
				</ul>
			</li>
			<li><a href="#variant">Variant Calling & Annotation & Prioritization</a></li>
			<li>
				<ul>
					<li><a href="#variant_calling">Calling</a></li>
					<li><a href="#variant_annotation_prioritization">Annotation & Prioritization</a></li>
				</ul>
			</li>
	...
	<section>
		...
		<article>
			<header>
				<h2 id="main">Main Information</h2>
			</header>
			<p>'$(cat $HTMLFILE.main)'</p>
		</article>
		...
		<article>
			...
			<p>'$(cat $HTMLFILE.mapping)'</p>

			<header>
				<h3 id="target_coverage">Target Coverage</h3>
			</header>
			<p>'$(cat $HTMLFILE.target_coverage)'</p>

			<header>
				<h3 id="gene_coverage">Gene Coverage</h3>
			</header>
			<p>'$(cat $HTMLFILE.gene_coverage)'</p>

		</article>

		<article>
			<header>
				<h2 id="variant">Variant Calling & Annotation & Prioritization</h2>
			</header>

			<header>
				<h3 id="variant_calling">Calling</h3>
			</header>
			<p>'$(cat $HTMLFILE.variant_calling)'</p>

			<header>
				<h3 id="variant_annotation_prioritization">Annotation & Prioritization</h3>
			</header>
			<p>'$(cat $HTMLFILE.variant_annotation_prioritization)'</p>

		</article>

		<article>
			<header>
				<h2 id="variant">Annexes</h2>
			</header>
			<p>'$(cat $HTMLFILE.annexe)'</p>
	...
	<footer>
		<p>'$ENV_NAME' ['$ENV_RELEASE'-'$ENV_DATE'] - '$ENV_DESCRIPTION' - (C)'$ENV_COPYRIGHT' - '$ENV_LICENCE'</p>
	</footer>
...
xvfb-run wkhtmltopdf -B $MARGIN -T $MARGIN --zoom 0.5 --no-background $HTMLFILE $HTMLPDFFILE
#echo xvfb-run wkhtmltopdf -B $MARGIN -T $MARGIN $HTMLFILE $PDFFILE
echo $HTMLPDFFILE



#if [ ! -z $OUTPUT_PDFFILE ] && [ -e $HTMLPDFFILE ]; then
if [ -e $HTMLPDFFILE ]; then
	cp $HTMLPDFFILE $OUTPUT_HTMLPDFFILE;
	cp $HTMLFILE $OUTPUT_HTMLPDFFILE.html;
	cp -R $REPORTDIR/$HTMLFOLDER $(dirname $OUTPUT_HTMLPDFFILE.html)/
fi;
