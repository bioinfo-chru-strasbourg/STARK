...
export STARK_FOLDER_APPS=$STARK_FOLDER_ROOT/apps
...
mkdir -p $DEMULTIPLEXING_FOLDER $RESULTS_FOLDER $ANALYSIS_FOLDER $TMP_FOLDER_TMP $VALIDATION_FOLDER $TMP_FOLDER $MAIN_FOLDER 2>/dev/null

# TOOLS FOLDER
export NGS_TOOLS=$FOLDER_TOOLS				# TOOLS
export DBFOLDER=$FOLDER_DATABASES			# DB
export NGS_GENOMES=$DBFOLDER/genomes			# GENOMES folder
export RULES=$STARK_FOLDER_APPS/rules.mk/*.rules.mk	# RULES
...
if [ ! -z "$(ls $STARK_FOLDER_APPS/$APP_NAME.rules.mk/*rules.mk 2>/dev/null)" ]; then
	RULES_APP="$RULES_APP $STARK_FOLDER_APPS/$APP_NAME.rules.mk/*.rules.mk"
fi;
