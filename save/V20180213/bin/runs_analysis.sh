...
do
	...
	if [ $SAMPLE_SHEET_MISEQ_EXISTS -eq 0 ]; then
		echo "["`date '+%Y%m%d-%H%M%S'`"] ERROR: SampleSheet for RUN '$RUN' missing! File '$SAMPLE_SHEET_MISEQ_ORIGINAL' doesn't exist"
		exit 1;
	else
		echo "["`date '+%Y%m%d-%H%M%S'`"] SampleSheet for RUN '$RUN': '$SAMPLE_SHEET_MISEQ_ORIGINAL'"
	fi
...
do
	...
	then
		...
		make -j $THREADS -e NGSEnv=$NGS_FOLDER ENV=$ENV PARAM=$MAKEFILE_ANALYSIS_RUN $THREAD_PARAMETERS OUTDIR=$RES_SOURCE RELEASE=$RELEASE_RUN SNAPSHOT=$SNAPSHOT  FINAL_REPORT=$FINAL_REPORT_RUN FINAL_REPORT_FULL=$FINAL_REPORT_FULL_RUN ANALYSIS_DATE=$DATE_MIN -f $NGS_SCRIPTS/NGSWorkflow.mk $MAKE_DEBUG 1>>$LOGFILE_RES_RUN 2>>$LOGFILE_RES_RUN
...
do
	...
	RESULTS_FOLDER_COPY_FOLDER_RUN_STARKCopyComplete_list=""
	...
	do
		...
		#echo "RESULTS_FOLDER_COPY_ALL=$RESULTS_FOLDER_COPY_ALL"
		...
		if [ "$RESULTS_FOLDER_COPY_ALL" != "$RESULTS_FOLDER" ] && [ "$RESULTS_FOLDER_COPY_ALL" != "" ] ; then
			...
			do
				...
				while ! $COMMAND_COPY $RESULTS_FOLDER/$RUN/$SAMPLE/* $RESULTS_FOLDER_COPY_FOLDER/$RUN/$SAMPLE/$RESULTS_SUBFOLDER_DATA 1>>$LOGFILE_ANA 2>>$LOGFILE_ANA ; do : ; done;
				...
				while ! $COMMAND_COPY $(find $RESULTS_FOLDER/$RUN -mindepth 1 -maxdepth 1 -type f) $RESULTS_FOLDER_COPY_FOLDER/$RUN 1>>$LOGFILE_ANA 2>>$LOGFILE_ANA ; do : ; done;
				...
				if [ $RESULTS_SUBFOLDER_DATA != "" ]; then
					for ROOT_FILE_PATTERN in $RESULTS_SUBFOLDER_ROOT_FILE_PATTERNS; do
						echo "["`date '+%Y%m%d-%H%M%S'`"] Copying $ROOT_FILE_PATTERN..."
						
						# Find pattern '$SAMPLE' and replace to SAMPLE calue
						...
						for F_SOURCE in $(ls $RESULTS_FOLDER_COPY_FOLDER/$RUN/$SAMPLE/$RESULTS_SUBFOLDER_DATA/$ROOT_FILE_PATTERN_VAR); do
							L_SOURCE=$(echo $F_SOURCE | sed s#$RESULTS_FOLDER_COPY_FOLDER/$RUN/$SAMPLE/##g)
							F_TARGET="$RESULTS_FOLDER_COPY_FOLDER/$RUN/$SAMPLE/"$(basename $F_SOURCE)
							#D_TARGET="$RESULTS_FOLDER_COPY_FOLDER/$RUU/$S/"
							#echo "F_SOURCE: $F_SOURCE";
							#echo "L_SOURCE: $L_SOURCE";
							#echo "F_TARGET: $F_TARGET";
							if [ ! -e $F_TARGET ]; then
								$COMMAND_LINK $L_SOURCE $F_TARGET 1>>$LOGFILE_ANA 2>>$LOGFILE_ANA
							fi;
							if [ ! -e $F_TARGET ]; then
								rm -f $F_TARGET
								while ! $COMMAND_COPY $F_SOURCE $F_TARGET 1>>$LOGFILE_ANA 2>>$LOGFILE_ANA ; do : ; done;
							fi;
						done;
				...
				echo "["`date '+%Y%m%d-%H%M%S'`"] Copy complete" >> $RESULTS_FOLDER_COPY_FOLDER/$RUN/$SAMPLE/STARKCopyComplete.txt
				chmod $PERMS $RESULTS_FOLDER_COPY_FOLDER/$RUN/$SAMPLE/STARKCopyComplete.txt 1>/dev/null 2>/dev/null
				RESULTS_FOLDER_COPY_FOLDER_RUN_STARKCopyComplete_list="$RESULTS_FOLDER_COPY_FOLDER_RUN_STARKCopyComplete_list $RESULTS_FOLDER_COPY_FOLDER/$RUN"
		...
		if [ "$RESULTS_FOLDER_LINK" != "$RESULTS_FOLDER" ] && [ "$RESULTS_FOLDER_LINK" != "" ] ; then
			...
			do
				...
				if [ ! -s $RESULTS_FOLDER_LINK_FOLDER/$RUN/$SAMPLE ]; then
					$COMMAND_LINK $RESULTS_FOLDER/$RUN/$SAMPLE/ $RESULTS_FOLDER_LINK_FOLDER/$RUN/$SAMPLE 1>>$LOGFILE_ANA 2>>$LOGFILE_ANA;
				fi;
	...
	if [ "$RESULTS_FOLDER_COPY" != "$RESULTS_FOLDER" ] && [ "$RESULTS_FOLDER_COPY" != "" ] ; then
		...
		do
			...
			while ! find $RESULTS_FOLDER/$RUN -mindepth 1 -maxdepth 1 -type f -exec $COMMAND_COPY -t $RESULTS_FOLDER_COPY_FOLDER/$RUN {} +  1>>$LOGFILE_ANA 2>>$LOGFILE_ANA ; do : ; done;
		 	#$COMMAND_COPY $RESULTS_FOLDER/$RUN/* $RESULTS_FOLDER_COPY_FOLDER/$RUN 1>>$LOGFILE_ANA 2>>$LOGFILE_ANA #&
		 	
		 	# CREATE CopyComplete file
			echo "["`date '+%Y%m%d-%H%M%S'`"] Copy complete" >> $RESULTS_FOLDER_COPY_FOLDER/$RUN/STARKCopyComplete.txt
			chmod $PERMS $RESULTS_FOLDER_COPY_FOLDER/$RUN/STARKCopyComplete.txt 1>/dev/null 2>/dev/null
	
		done;
	fi;
	
	# CREATE CopyComplete file
	for RESULTS_FOLDER_COPY_FOLDER_RUN_STARKCopyComplete in $RESULTS_FOLDER_COPY_FOLDER_RUN_STARKCopyComplete_list; do
		echo "["`date '+%Y%m%d-%H%M%S'`"] Copy complete" >> $RESULTS_FOLDER_COPY_FOLDER_RUN_STARKCopyComplete/STARKCopyComplete.txt
		chmod $PERMS $RESULTS_FOLDER_COPY_FOLDER_RUN_STARKCopyComplete/STARKCopyComplete.txt 1>/dev/null 2>/dev/null
	done;
...
do
	if [ "$VARANK_ANALYSIS" == 1 ] ; then
		...
		$NGS_SCRIPTS/launch.VaRank.sh -r $RESULTS_FOLDER/$RUN 1>/dev/null 2>/dev/null
	fi;
done
...
if [ $USE_TMP_FOLDER -gt 0 ]; then
	...
	if [ -d $TMP_ANA_DIR ]; then
		...
		chmod $PERMS -R $TMP_ANA_DIR/*  1>/dev/null 2>/dev/null
	 	...
	 	while ! $COMMAND_COPY $TMP_ANA_DIR/* $ANALYSIS_FOLDER 1>>$LOGFILE_ANA 2>>$LOGFILE_ANA ; do : ; done;
	fi;
