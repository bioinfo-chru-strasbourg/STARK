...
# 0.9.4-03/02/2023: Extract gatkUG SOLIDTUMOR
...
DFRAC_UG_SOLIDTUMOR=1
MBQ_UG_SOLIDTUMOR=17
THREADS_GATKUG_SOLIDTUMOR?=$(THREADS_BY_SAMPLE)
DPMIN_UG_SOLIDTUMOR=1
...
%.gatkUG_SOLIDTUMOR$(POST_CALLING).vcf: %.bam %.bam.bai %.empty.vcf %.design.bed.interval_list
	...
	$(BCFTOOLS) view -i "FORMAT/DP>$(DPMIN_UG_SOLIDTUMOR)" $@.tmp > $@ 		# filter on DP, cause UG is too relax, espacially because the clipping can generate few errors
	-if [ ! -e $@ ]; then cp $@.tmp $@; fi; 								# in case of error in previous line
	-rm -f $@.tmp $@.tmp.idx $@.idx



RELEASE_COMMENT := "\#\# CALLING GATKUG SOLIDTUMOR identify variants and generate *.gatkUG_SOLIDTUMOR.vcf files with parameters: GATKUG_SOLIDTUMOR_FLAGS='$(GATKUG_SOLIDTUMOR_FLAGS)', INTERVAL_PADDING='$(INTERVAL_PADDING)', DPMIN_UG_SOLIDTUMOR='$(DPMIN_UG_SOLIDTUMOR)'"
RELEASE_CMD := $(shell echo "$(RELEASE_COMMENT)" >> $(RELEASE_INFOS) )

PIPELINES_COMMENT := "CALLER:gatkUG_SOLIDTUMOR:GATK Unified Genotyper - designed for SOLIDTUMOR variant discovery:GATKUG_SOLIDTUMOR_FLAGS='$(GATKUG_SOLIDTUMOR_FLAGS)', INTERVAL_PADDING='$(INTERVAL_PADDING)', DPMIN_UG_SOLIDTUMOR='$(DPMIN_UG_SOLIDTUMOR)'"
