...
# 0.9.4-03/02/2023: Extract gatkUG GERMLINE
...
%.gatkUG_GERMLINE$(POST_CALLING).vcf: %.bam %.bam.bai %.empty.vcf %.design.bed.interval_list
	...
	$(BCFTOOLS) view -i "FORMAT/DP>$(DPMIN_UG_GERMLINE)" $@.tmp > $@ 	# filter on DP, cause UG is too relax, espacially because the clipping can generate few errors
	-if [ ! -e $@ ]; then cp $@.tmp $@; fi; 							# in case of error in previous line
	-rm -f $@.tmp $@.tmp.idx $@.idx



RELEASE_COMMENT := "\#\# CALLING GATKUG GERMLINE identify variants and generate *.gatkUG_GERMLINE.vcf files with parameters: GATKUG_GERMLINE_FLAGS='$(GATKUG_GERMLINE_FLAGS)', INTERVAL_PADDING='$(INTERVAL_PADDING)', DPMIN_UG_GERMLINE='$(DPMIN_UG_GERMLINE)'"
RELEASE_CMD := $(shell echo "$(RELEASE_COMMENT)" >> $(RELEASE_INFOS) )

PIPELINES_COMMENT := "CALLER:gatkUG_GERMLINE:GATK Unified Genotyper - designed for GERMLINE discovery:GATKUG_GERMLINE_FLAGS='$(GATKUG_GERMLINE_FLAGS)', INTERVAL_PADDING='$(INTERVAL_PADDING)', DPMIN_UG_GERMLINE='$(DPMIN_UG_GERMLINE)'"
