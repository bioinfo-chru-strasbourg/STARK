...
%.VarScan_EXOME_SOMATIC$(POST_CALLING).SNP.vcf: %.bam.mpileup %.empty.vcf #
	if [ -s $< ]; then \
		sample=`basename $* | cut -d"." -f1`; \
		echo $$sample > $*.SNP.sample.txt; \
		cat $< | $(JAVA) -jar $(VARSCAN) mpileup2snp --output-vcf 1 $(VARSCAN_EXOME_SOMATIC_SNP_OPTIONS) --vcf-sample-list $*.SNP.sample.txt > $@; \
		rm -f $*.SNP.sample.txt; \
	fi;
...
%.VarScan_EXOME_SOMATIC$(POST_CALLING).InDel.vcf: %.bam.mpileup %.empty.vcf
	if [ -s $< ]; then \
		sample=`basename $* | cut -d"." -f1`; \
		echo $$sample > $*.InDel.sample.txt; \
		cat $< | $(JAVA) -jar $(VARSCAN) mpileup2indel --output-vcf 1 $(VARSCAN_EXOME_SOMATIC_INDEL_OPTIONS) --vcf-sample-list $*.InDel.sample.txt > $@; \
