...
# 19/01/2022: Changing UMI options
...
%.bam: %.markduplicates.bam %.markduplicates.bam.bai
	...
	if (( $$($(SAMTOOLS) view -H $< | grep "^@HD.*VN:.*SO:coordinate" | wc -l))); then \
		...
		$$(if [ "$(UMI_TYPE)" == "duplex" ]; then echo "DUPLEX_UMI=True"; fi;) \
...
RELEASE_COMMENT := "\#\# MARK_DUPLICATES: Mark duplicated reads in BAM with PICARD MarkDuplicates. UMI options. Not parallelized"
