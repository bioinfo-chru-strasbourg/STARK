...
%.VarScan_EXOME_SOMATIC.SNP.vcf: %.bam.mpileup %.from_manifest.intervals %.empty.vcf %.genome #
	-if [ -s $< ]; then \
		echo "# GENERATION of '$@' start: "`date`; \
		sample=`basename $* | cut -d"." -f1`; \
		echo $$sample > $*.SNP.sample.txt; \
...
%.VarScan_EXOME_SOMATIC.InDel.vcf: %.bam.mpileup %.from_manifest.intervals %.empty.vcf %.genome
	-if [ -s $< ]; then \
		echo "# GENERATION of '$@' start: "`date`; \
		sample=`basename $* | cut -d"." -f1`; \
		echo $$sample > $*.InDel.sample.txt; \
