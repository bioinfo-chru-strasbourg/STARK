...
%.bam: %.clipping.bam %.clipping.bam.bai %.manifest %.genome
	# Clipping
	+$(CAP) --function=clipping --env=$(CONFIG_TOOLS) --ref=$$(cat $*.genome) --bam=$< --manifest=$*.manifest --output=$@ --threads=$(CAP_THREADS) --bedtools=$(BEDTOOLS) --samtools=$(SAMTOOLS) --picard=$(PICARD) --tmp=$(CAP_TMP_FOLDER) $(CAP_OPTIONS)
	#-rm $*.unclipped.*
	-rm $*.clipping.*
