...
ENV YUM_INSTALL="autoconf automake htop bc bzip2 bzip2-devel curl gcc gcc-c++ git make ncurses-devel perl perl-Data-Dumper perl-Digest-MD5 perl-Switch perl-devel perl-Tk tbb-devel unzip rsync wget which xz xz-devel zlib zlib-devel docker java-1.8.0-openjdk java-11-openjdk python3 python3-pip python3-devel libcurl-devel openssl-devel"

ENV YUM_REMOVE="autoconf automake bzip2-devel ncurses-devel perl-devel tbb-devel xz-devel zlib-devel zlib2-devel python3-devel curl-devel openssl-devel"
...
# almalinux comes with curl-minimal ; upgrading it to curl requires a special option, treat it separately from other packages
RUN dnf install -y curl --allowerasing

# RUN echo "#[INFO] System installation - and download" && \
...
# 	echo "#[ERROR] System wget package not installed (Please open Internet connection or provide WGET rpm in sources/system folder)"; \
# 	exit 1; \
# 	fi && \
# 	if ! command -v rsync 1>/dev/null 2>/dev/null; then \
# 	echo "#[ERROR] System rsync package not installed (Please open Internet connection or provide RSYNC rpm in sources/system folder)"; \
# 	exit 1; \
# 	fi && \
...
RUN yum $YUM_PARAM install -y --downloadonly --downloaddir=$SOURCES/$SOURCES_FOLDER/system/build/install/ $YUM_INSTALL && \
...
# PYTHON installation
#this makes both the python and python3 aliases refer to python 3
...
ENV TOOL_SOURCE_EXTERNAL="https://github.com/samtools/htslib/releases/download/$TOOL_VERSION/$TOOL_TARBALL"
# TOOL PARAMETERS

# TOOL INSTALLATION
RUN echo "#[INFO] TOOL installation '$TOOL_NAME:$TOOL_VERSION'" && \
	wget $TOOL_SOURCE_EXTERNAL && \
	tar -vxjf $TOOL_TARBALL && \
...
# TOOL INFO
ENV TOOL_NAME="bwa"
ENV TOOL_VERSION="0.7.19"
ENV TOOL_TARBALL="$TOOL_NAME-$TOOL_VERSION.tar.bz2"
ENV TOOL_SOURCE_EXTERNAL="https://github.com/lh3/bwa/archive/refs/tags/v$TOOL_VERSION.zip"
ENV PATH=$TOOLS/$TOOL_NAME/$TOOL_VERSION/bin:$PATH
# TOOL PARAMETERS
...
RUN echo "#[INFO] TOOL installation '$TOOL_NAME:$TOOL_VERSION'" && \
	...
	mv $TOOL_NAME-$TOOL_VERSION $TOOLS/$TOOL_NAME/$TOOL_VERSION/bin
