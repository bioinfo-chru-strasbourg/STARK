...
VCF=	$(foreach RUN_SAMPLE,$(RUNS_SAMPLES),$(foreach ALIGNER,$(ALIGNERS),$(foreach CALLER,$(CALLERS),$(foreach ANNOTATOR,$(ANNOTATORS),$(OUTDIR)/$(call run,$(RUN_SAMPLE))/$(call sample,$(RUN_SAMPLE))/$(call sample,$(RUN_SAMPLE)).$(ALIGNER).$(CALLER).$(ANNOTATOR).vcf )))) \
	...
	$(foreach RUN_SAMPLE,$(RUNS_SAMPLES),$(foreach ALIGNER,$(ALIGNERS),$(foreach CALLER,$(CALLERS),$(foreach ANNOTATOR,$(ANNOTATORS),$(OUTDIR)/$(call run,$(RUN_SAMPLE))/$(call sample,$(RUN_SAMPLE))/$(call sample,$(RUN_SAMPLE)).$(ALIGNER).$(CALLER).$(ANNOTATOR).txt )))) \
	...
	$(foreach RUN_SAMPLE,$(RUNS_SAMPLES),$(foreach PIPELINE,$(PIPELINES),$(OUTDIR)/$(call run,$(RUN_SAMPLE))/$(call sample,$(RUN_SAMPLE))/$(call sample,$(RUN_SAMPLE)).$(PIPELINE).txt ))
...
FINAL=$(SAMPLE) $(BAM) $(VCF) $(CRAM) $(FASTQC_METRICS)
#$(UBAM)
...
REPORT_FILES_EXT= final.vcf full.vcf final.txt full.txt $(ANALYSIS_DATE).final.vcf $(ANALYSIS_DATE).full.vcf $(ANALYSIS_DATE).final.txt $(ANALYSIS_DATE).full.txt
#full.txt 
...
all: $(FINAL) $(FINAL_REPORT) $(FINAL_REPORT_FILES) #$(FINAL_REPORT_FULL) $(FINAL_REPORT_FULL_VCF)
...
#%.vcfs.list: $(foreach ALIGNER,$(ALIGNERS),$(foreach CALLER,$(CALLERS),%.$(ALIGNER).$(CALLER).vcf))
