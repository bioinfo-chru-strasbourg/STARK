...
do
	...
	#echo $COVERAGEFILESTATS_SAMTOOLS
	#ls $COVERAGEFILESTATS_SAMTOOLS
	#echo $COVERAGEFILESTATS_HSMETRICS
	#ls $COVERAGEFILESTATS_HSMETRICS
	...
	if [ -e $COVERAGEFILESTATS_ONREADS_SAMTOOLS ] && [ -e $COVERAGEFILESTATS_OFFREADS_SAMTOOLS ]; then
		...
		if [ "$READS_ON_TARGET" != "" ] && [ "$READS_OFF_TARGET" != "" ]; then
			...
			\end{table}" >> $TEXFILE
			#$SAMPLEID & `echo "scale=2;($BASES_ON_TARGET*100)/1" | bc` & `echo "scale=2;($BASES_OFF_TARGET*100)/1" | bc` \\\\
			# $SAMPLEID & $BASES_ON_TARGET & `echo "scale=2;(($BASES_ON_TARGET/$BASES_ALL_TARGET)*100)/1" | bc`\% & $BASES_OFF_TARGET & `echo "scale=2;(($BASES_OFF_TARGET/$BASES_ALL_TARGET)*100)/1" | bc`\% \\\\
		fi;
	...
	if ((0)); then
	if [ -e $COVERAGEFILESTATS_SAMTOOLS ] && [ -e $COVERAGEFILESTATSOFF_SAMTOOLS ]; then
		BASES_ON_TARGET=$(cat $COVERAGEFILESTATS_SAMTOOLS | awk '{SUM+=$3} END {print SUM+0}')
		BASES_OFF_TARGET=$(cat $COVERAGEFILESTATSOFF_SAMTOOLS | awk '{SUM+=$3} END {print SUM+0}')
		BASES_ALL_TARGET=$(echo "$BASES_ON_TARGET+$BASES_OFF_TARGET" | bc)
		
		if [ "$BASES_ON_TARGET" != "" ] && [ "$BASES_OFF_TARGET" != "" ]; then
			...
			\end{tabular}
			\end{center}
			\caption{\label{stats_on_off_$aligner}Coverage statistics between on and off targets bases on $aligner on $SAMPLEID. Statistics were determined with SAMTOOLS/BEDTOOLS (duplicates and bad quality reads removed) on the $SAMPLEID.$aligner.bam file.}
			\end{table}" >> $TEXFILE
			#$SAMPLEID & `echo "scale=2;($BASES_ON_TARGET*100)/1" | bc` & `echo "scale=2;($BASES_OFF_TARGET*100)/1" | bc` \\\\
			# $SAMPLEID & $BASES_ON_TARGET & `echo "scale=2;(($BASES_ON_TARGET/$BASES_ALL_TARGET)*100)/1" | bc`\% & $BASES_OFF_TARGET & `echo "scale=2;(($BASES_OFF_TARGET/$BASES_ALL_TARGET)*100)/1" | bc`\% \\\\
		fi;
		...
		echo "No depthBED ??? '$COVERAGEFILESTATS_SAMTOOLS' '$COVERAGEFILESTATSOFF_SAMTOOLS'";
	...
	#if ((1)); then
	#COVERAGEFILESTATS_SAMTOOLS="$RESULTS_FOLDER/$FLOWCELL/$SAMPLE/$SAMPLE.$aligner.bam.metrics/$SAMPLE.$aligner.HsMetrics"
	#if [ -s $RESULTS_FOLDER/$FLOWCELL/$SAMPLE/$SAMPLE.$aligner.bam.metrics/$SAMPLE.$aligner.HsMetrics ]
	...
	then
		...
		#cat $COVERAGEFILESTATS_SAMTOOLS | awk -v MDP=$MDP '{SUM++} { if ($3>MDP) {DP[MDP]++} else {DP[$3]++} } END { for (i in DP) {print i" "DP[i]" SUM"SUM}}' | sort -g -r | awk '{SUM+=$2} {CUM[$1]=SUM}  END { for (j in CUM) {print j" "CUM[j]" "SUM" "(CUM[j]/SUM)} }' | sort -g > $COVERAGEFILESTATS_SAMTOOLS.summary.tmp
		#cat $COVERAGEFILESTATS_SAMTOOLS | awk -v MDP=$MDP '{SUM++} { if ($3>MDP) {DP[MDP]++} else {DP[$3]++} } END { for (i in DP) {print i" "DP[i]" SUM"SUM}}' | sort -g -r | awk '{SUM+=$2} {CUM[$1]=SUM}  END { for (j = 1; j <= MDP; j++) {print j" "CUM[j]" "SUM" "(CUM[j]/SUM)} }' | sort -g > $COVERAGEFILESTATS_SAMTOOLS.summary.tmp
		#echo "cat $COVERAGEFILESTATS_SAMTOOLS | awk -v MDP=$MDP '{SUM++} { if ($3>MDP) {DP[MDP]++} else {DP[$3]++} } END { for (i=MDP; i>=0; i-=1) {print i" "DP[i]" SUM"SUM}}' | sort -g -r | awk -v MDP=$MDP -v STEP=$STEP '{SUM+=$2} {CUM[$1]=SUM}  END { for (j = 0; j <=MDP; j+=STEP) {print j" "CUM[j]" SUM "(CUM[j]/SUM)} }' | sort -g > $COVERAGEFILESTATS_SAMTOOLS.summary.tmp"
		#cat $COVERAGEFILESTATS_SAMTOOLS | awk -v MDP=$MDP '{SUM++} { if ($3>MDP) {DP[MDP]++} else {DP[$3]++} } END { for (i=MDP; i>=0; i-=1) {print i" "DP[i]" SUM"SUM}}' | sort -g -r | awk -v MDP=$MDP -v STEP=$STEP '{SUM+=$2} {CUM[$1]=SUM}  END { for (j = 0; j <=MDP; j+=STEP) {print j" "CUM[j]" SUM "(CUM[j]/SUM)} }' | sort -g > $COVERAGEFILESTATS_SAMTOOLS.summary.tmp
		...
		echo "
			...
			" >> $TEXFILE
			# \caption{Coverage statistics on targeted regions on $aligner on $SAMPLEID. Statistics were determined with SAMTOOLS on the $SAMPLEID.$aligner.bam file, from 0 to $MDP (STEP=$STEP).} \label{tab:long} \\\\	
		# \label{tab:long} \\\\
		if ((0)); then
			echo "\begin{table}[H]
			\begin{center}
			...
			echo "\end{tabular}
			\end{center}
			\caption{\label{stats_on_off_$aligner}Coverage statistics on targets region on $aligner on $SAMPLEID. Statistics were determined with Picard HSMetrics on the $SAMPLEID.$aligner.bam file.}
			\end{table}" >> $TEXFILE
		fi;
	...
	# COVERAGE SUMMARY with HsMetrics
	if ((0)); then
	COVERAGEFILESTATS="$RESULTS_FOLDER/$FLOWCELL/$SAMPLE/$SAMPLE.$aligner.bam.metrics/$SAMPLE.$aligner.HsMetrics"
	#if [ -s $RESULTS_FOLDER/$FLOWCELL/$SAMPLE/$SAMPLE.$aligner.bam.metrics/$SAMPLE.$aligner.HsMetrics ]
	if [ -s $COVERAGEFILESTATS ]
	then
	
		
		#if [ -e $COVERAGEFILESTATS ]; then
			PCT_SELECTED_BASES=$( C=1; for i in $(grep -v "#" $COVERAGEFILESTATS | sed '/^\s*$/d' | head -n 2) ; do if [ $i == "PCT_SELECTED_BASES" ] ; then break ; else C=$(( $C + 1 )) ; fi ; done ; grep -v "#" $COVERAGEFILESTATS | sed '/^\s*$/d'  | head -n2 | tail -n1 | cut -f$C | sed 's/,/./' )
			#PCT_SELECTED_BASES=$( C=1; for i in $(grep -v "#" $COVERAGEFILESTATS | sed '/^\s*$/d' | head -n 2) ; do if [ $i == "PCT_SELECTED_BASES" ] ; then break ; else C=$(( $C + 1 )) ; fi ; done ; grep -v "#" $COVERAGEFILESTATS | sed '/^\s*$/d' | head -n2 | tail -n1 | cut -f$C | sed 's/,/./' )
			PCT_OFF_BAIT=$( C=1; for i in $(grep -v "#" $COVERAGEFILESTATS | sed '/^\s*$/d' | head -n 2) ; do if [ $i == "PCT_OFF_BAIT" ] ; then break ; else C=$(( $C + 1 )) ; fi ; done ; grep -v "#" $COVERAGEFILESTATS | sed '/^\s*$/d'  | head -n2 | tail -n1 | cut -f$C | sed 's/,/./' )
			PCT_TARGET_BASES_10X=$( C=1; for i in $(grep -v "#" $COVERAGEFILESTATS | sed '/^\s*$/d' | head -n 2) ; do if [ $i == "PCT_TARGET_BASES_10X" ] ; then break ; else C=$(( $C + 1 )) ; fi ; done ; grep -v "#" $COVERAGEFILESTATS | sed '/^\s*$/d'  | head -n2 | tail -n1 | cut -f$C | sed 's/,/./' )
			PCT_TARGET_BASES_20X=$( C=1; for i in $(grep -v "#" $COVERAGEFILESTATS | sed '/^\s*$/d' | head -n 2) ; do if [ $i == "PCT_TARGET_BASES_20X" ] ; then break ; else C=$(( $C + 1 )) ; fi ; done ; grep -v "#" $COVERAGEFILESTATS | sed '/^\s*$/d'  | head -n2 | tail -n1 | cut -f$C | sed 's/,/./'  )
			PCT_TARGET_BASES_30X=$( C=1; for i in $(grep -v "#" $COVERAGEFILESTATS | sed '/^\s*$/d' | head -n 2) ; do if [ $i == "PCT_TARGET_BASES_30X" ] ; then break ; else C=$(( $C + 1 )) ; fi ; done ; grep -v "#" $COVERAGEFILESTATS | sed '/^\s*$/d'  | head -n2 | tail -n1 | cut -f$C | sed 's/,/./' )
			PCT_TARGET_BASES_40X=$( C=1; for i in $(grep -v "#" $COVERAGEFILESTATS | sed '/^\s*$/d' | head -n 2) ; do if [ $i == "PCT_TARGET_BASES_40X" ] ; then break ; else C=$(( $C + 1 )) ; fi ; done ; grep -v "#" $COVERAGEFILESTATS | sed '/^\s*$/d'  | head -n2 | tail -n1 | cut -f$C | sed 's/,/./' )
			PCT_TARGET_BASES_50X=$( C=1; for i in $(grep -v "#" $COVERAGEFILESTATS | sed '/^\s*$/d' | head -n 2) ; do if [ $i == "PCT_TARGET_BASES_50X" ] ; then break ; else C=$(( $C + 1 )) ; fi ; done ; grep -v "#" $COVERAGEFILESTATS | sed '/^\s*$/d'  | head -n2 | tail -n1 | cut -f$C | sed 's/,/./' )
			PCT_TARGET_BASES_100X=$( C=1; for i in $(grep -v "#" $COVERAGEFILESTATS | sed '/^\s*$/d' | head -n 2) ; do if [ $i == "PCT_TARGET_BASES_100X" ] ; then break ; else C=$(( $C + 1 )) ; fi ; done ; grep -v "#" $COVERAGEFILESTATS | sed '/^\s*$/d'  | head -n2 | tail -n1 | cut -f$C | sed 's/,/./'  )
		#fi;
	...
	Sample & $SAMPLEID  \\\\
	\hline
	\% target bases 10X & `echo "scale=2;($PCT_TARGET_BASES_10X*100)/1" | bc`  \\\\
	\hline
	\% target bases 20X & `echo "scale=2;($PCT_TARGET_BASES_20X*100)/1" | bc`  \\\\
	\hline 
	\% target bases 30X & `echo "scale=2;($PCT_TARGET_BASES_30X*100)/1" | bc`  \\\\
	\hline
	\% target bases 40X & `echo "scale=2;($PCT_TARGET_BASES_40X*100)/1" | bc`  \\\\
	\hline
	\% target bases 50X & `echo "scale=2;($PCT_TARGET_BASES_50X*100)/1" | bc`  \\\\
	\hline
	\% target bases 100X & `echo "scale=2;($PCT_TARGET_BASES_100X*100)/1" | bc`  \\\\
	\hline
	\end{tabular}
	\end{center}
	\caption{\label{stats_on_off_$aligner}Coverage statistics on targets region on $aligner on $SAMPLEID. Statistics were determined with Picard HSMetrics on the $SAMPLEID.$aligner.bam file.}
	...
	#echo "PCT_SELECTED_BASES=$PCT_SELECTED_BASES|";
	#echo "PCT_TARGET_BASES_10X=$PCT_TARGET_BASES_10X|";
	...
	#echo "scale=2;($PCT_SELECTED_BASES*100)/1" | bc
	#echo "$SAMPLE & `echo "scale=2;($PCT_SELECTED_BASES*100)/1" | bc` & `echo "scale=2;($PCT_OFF_BAIT*100)/1" | bc` \\\\"
...
FINALTXTFILE=$RESULTS_FOLDER/$FLOWCELL/$SAMPLE/$SAMPLE.reports/$SAMPLE.final.txt
...
if ((0)); then
	...
	echo "\section{Variants passing annotation filter}" >> $TEXFILE
	# (PZFlag=PASS) HGVS
	#echo "\begin{itemize}" >> $TEXFILE
	# List of Variants
	#grep -v ^# $FINALVCFFILE | awk -F "\t" '{print $8}' | tr ";" "\n"  | awk -F "=" '/hgvs/ {print $2}' | awk -F ","  '{print $1}' >> $TEXFILE

	echo "\begin{itemize}" >> $TEXFILE

		echo -n "\item ";
		grep -v ^# $FINALVCFFILE | grep "PZFlag=PASS" | while read line; do echo -n "\item "$(echo -ne $line | tr ";" "\n" | awk -F "=" '/hgvs/ {print $2}' | awk -F ","  '{print $1}' | head -n 1); echo ""; done | sort  >> $TEXFILE
...
if ((1)); then
	...
	echo "\begin{itemize}" >> $TEXFILE
	echo "\item Number of variants filtered by the prioritization filter '"$(echo $HOWARD_FILTER | cut -d, -f1)"': $NB_FILTERED" >> $TEXFILE
	echo "\item Number of variants passing the prioritization filter '"$(echo $HOWARD_FILTER | cut -d, -f1)"': $NB_PASS" >> $TEXFILE
	echo "\end{itemize}" >> $TEXFILE
	...
	echo "\begin{longtable}{|l|l|l|}
	\caption{Variants passing prioritization filter '"$(echo $HOWARD_FILTER | cut -d, -f1)"'.} \label{tab:long} \\\\

	\hline \multicolumn{1}{|c|}{\textbf{HGVS}} & \multicolumn{1}{c|}{\textbf{Location/Outcome}} & \multicolumn{1}{c|}{\textbf{VAF}} \\\\ \hline 
	\endfirsthead
...
if ((0)); then # OK
	echo "\section{Variants prioritization (first 40}" >> $TEXFILE
	echo "\begin{longtable}{|l|l|l|}
...
\endlastfoot
		...
		#awk -F "\t" '$9 == "PASS" {print $11" & "$13" & "$14" & \begin{left} "$12" \end{left} \\\\ \\hline"}' $FINALTXTFILE | sed "s/,/ /gi" | sed 's/_/\\_/gi' >> $TEXFILE
		#awk -F "\t" '$9 == "PASS" {print $11" & "$13" & "$14" & \\hline "$12" \\\\ \\hline"}' $FINALTXTFILE | cut -d, -f1 | sed 's/_/\\_/gi' >> $TEXFILE
		awk -F "\t" '{split($12,H,",")} $9 == "PASS" {print H[1]" & "$13" & "$14" \\\\ \\hline"}' $FINALTXTFILE | sed 's/_/\\_/gi' | head -n 40 >> $TEXFILE
...
if ((0)); then
	...
	echo "\begin{table}[10pt]
		...
		#echo " \\\\ \hline";
	echo "\end{tabular}
			\caption{Variants}\label{variants}
			\end{table}" >> $TEXFILE
fi;
