...
function usage {
	...
	echo "# -f/--fastq=<FILE1,FILE2,...>                List of FASTQ/BAM/SAM/CRAM (mandatory)";
	...
	echo "# -b/--bed=<FILE1,FILE2,...>                  List of corresponding BED files in BED format.";
...
ARGS=$(getopt -o "e:f:q:b:s:r:o:u:p:t:ga:vdnh" --long "env:,app:,fastq:,fastq_R2:,bed:,sample:,run:,output:,results:,repository:,pipelines:,threads:,by_sample,adapters:,verbose,debug,release,help" -- "$@" 2> /dev/null)
[ $? -ne 0 ] && \
...
if [ -s $ENV ] && [ "$ENV" != "" ] && [ ! -d $ENV ]; then
	ENV=$ENV;
elif [ -s "$(dirname "$FASTQ")/$ENV" ] && [ "$ENV" != "" ] && [ ! -d $ENV ]; then
	ENV="$(dirname "$FASTQ")"/$ENV;
elif [ -s $SCRIPT_DIR/$ENV ] && [ "$ENV" != "" ] && [ ! -d $ENV ]; then
	ENV=$SCRIPT_DIR/$ENV;
elif [ "$ENV" == "" ] || [ ! -s $ENV ]; then
	if [ -s "$(dirname "$FASTQ")"/"env.sh" ]; then
		ENV=$(dirname "$FASTQ")/"env.sh";
	elif [ $(ls "$(dirname "$FASTQ")"/env*.sh 2>/dev/null | wc -l ) -gt 0 ]; then
		ENV=$(ls $(dirname "$FASTQ")/env*.sh 2>/dev/null | head -n 1 );
	elif [ -s $SCRIPT_DIR/"env.sh" ]; then
...
if [ "$BED" != "" ]; then
	...
	BED_DEFAULT=$(echo $BED | cut -d" " -f1) #`date '+%Y%m%d-%H%M%S'`
...
if (($BY_SAMPLE)); then
	if [ $(echo $FASTQ | wc -w ) -gt 1 ]; then
		...
		#FASTQ_R2_ARRAY=($FASTQ_R2);
		#SAMPLE_ARRAY=($SAMPLE);
		PARAM_MULTI=$(echo $PARAM | sed "s/--by_sample[^ |$]*//gi" | sed s/--$//gi)
		i=0
		for f in ${FASTQ_ARRAY[@]}; do
			...
			$SCRIPT_DIR/$(basename $0) $PARAM_MULTI --fastq=$f --fastq_R2="${FASTQ_R2_ARRAY[$i]}" --sample="${SAMPLE_ARRAY[$i]}" --run="${RUN_ARRAY[$i]}" --bed="${BED_ARRAY[$i]}" ;
			((i++))
		done;
		exit 0;
	fi;
...
for RUU in $RUN_UNIQ; do
	...
	RELEASE_RUN=$OUTPUT/$RUU/$ANALYSIS_REF
	...
	for F in $FASTQ; do
		...
		if [ ! -s $RUN_SAMPLE_DIR/$S.R1.fastq.gz ]; then
			...
			# BAM/CRAM/SAM
				...
				$GZIP $RUN_SAMPLE_DIR/$S.R1.fastq --fast -f -q 1>/dev/null 2>/dev/null;
				$GZIP $RUN_SAMPLE_DIR/$S.R2.fastq --fast -f -q 1>/dev/null 2>/dev/null;
		...
		# DATA entry format is unaligned.bam !		
		if ((0)); then
							
		if [ ! -s $RUN_SAMPLE_DIR/$S.unaligned.bam ]; then


			# CREATE UBAM
			if (($(echo "$F" | grep ".fastq.gz$\|.fq.gz$" -c))); then
				if [ ! -e $RUN_SAMPLE_DIR/$S.fastq.gz ]; then
					# Create UBAM from fastq
					echo "#[INFO] Create uBAM from FASTQ file(s)"
					# TRIM ADAPTERS
					if [ "$ADAPTERS" != "" ]; then
					echo "#[INFO] Trim adapters."
						if [ -s $F ]; then
						...
						# Create FASTQ
						cat $F > $RUN_SAMPLE_DIR/$S.R1.fastq.gz;
						if [ -s $F_R2 ] && [ -e $F_R2 ]; then
							cat $F_R2 >> $RUN_SAMPLE_DIR/$S.R2.fastq.gz;
						fi;
					fi;
					#PICARD_UNALIGNED_NAME_FLAGS="COMPRESSION_LEVEL=1 LIBRARY_NAME=001 READ_GROUP_NAME=A PLATFORM=PL PLATFORM_UNIT=PU"
					if [ -s $F_R2 ] && [ -e $F_R2 ]; then
						$JAVA $JAVA_FLAGS -jar $PICARD FastqToSam FASTQ=$RUN_SAMPLE_DIR/$S.R1.fastq.gz FASTQ2=$RUN_SAMPLE_DIR/$S.R2.fastq.gz OUTPUT=$RUN_SAMPLE_DIR/$S.unaligned.bam SAMPLE_NAME=$S $PICARD_FLAGS $PICARD_UNALIGNED_FLAGS $PICARD_UNALIGNED_NAME_FLAGS 1>/dev/null 2>/dev/null;
					else
						$JAVA $JAVA_FLAGS -jar $PICARD FastqToSam FASTQ=$RUN_SAMPLE_DIR/$S.R1.fastq.gz OUTPUT=$RUN_SAMPLE_DIR/$S.unaligned.bam $PICARD_FLAGS $PICARD_UNALIGNED_FLAGS $PICARD_UNALIGNED_NAME_FLAGS 1>/dev/null 2>/dev/null;
					fi;

				fi;
			elif (($(echo $F | grep ".bam$\|.ubam$\|.cram$\|.ucram\|.sam$\|.usam$" -c))); then
					# Create UBAM from fastq
					echo "#[INFO] Create uBAM from BAM/CRAM/SAM file"
				
				if [ ! -e $RUN_SAMPLE_DIR/$S.unaligned.bam ]; then
					$SAMTOOLS view -F 0x0100 -o $RUN_SAMPLE_DIR/$S.unaligned.bam -O BAM -1 -S $F -@ $THREADS 1>/dev/null 2>/dev/null;
				fi;
			else

				echo "[ERROR] Unknown File format '$F'!";
				exit 0;	
			fi;
		...
		# Unaligned BAM Validation

		# SHORT validation on read groups
		if ((0)); then
		echo "#[INFO] Validation uBAM"
		$JAVA $JAVA_FLAGS -jar $PICARD ValidateSamFile $PICARD_FLAGS $PICARD_VALIDATE_FLAGS $PICARD_VALIDATE_NAME_FLAGS VALIDATE_INDEX=true IGNORE_WARNINGS=true INDEX_VALIDATION_STRINGENCY=EXHAUSTIVE I=$RUN_SAMPLE_DIR/$S.unaligned.bam > $RUN_SAMPLE_DIR/$S.unaligned.bam.validation 2>/dev/null;
		if [ $(grep "^ERROR:.*READ_GROUP.*" $RUN_SAMPLE_DIR/$S.unaligned.bam.validation -c) -gt 0 ] \
		|| [ $(grep "^ERROR:.*PLATFORM.*" $RUN_SAMPLE_DIR/$S.unaligned.bam.validation -c) -gt 0 ];
		then
			echo "#[INFO] Try to fix uBAM Read Group Information "
			$JAVA $JAVA_FLAGS -jar $PICARD AddOrReplaceReadGroups $PICARD_FLAGS $PICARD_ADDORREPLACEREADGROUPS_FLAGS $PICARD_ADDORREPLACEREADGROUPS_NAME_FLAGS I=$RUN_SAMPLE_DIR/$S.unaligned.bam O=$RUN_SAMPLE_DIR/$S.unaligned.tmp.bam 1>/dev/null 2>/dev/null
			$JAVA $JAVA_FLAGS -jar $PICARD ValidateSamFile $PICARD_FLAGS $PICARD_VALIDATE_FLAGS $PICARD_VALIDATE_NAME_FLAGS VALIDATE_INDEX=true IGNORE_WARNINGS=true INDEX_VALIDATION_STRINGENCY=EXHAUSTIVE I=$RUN_SAMPLE_DIR/$S.unaligned.tmp.bam > $RUN_SAMPLE_DIR/$S.unaligned.bam.validation2 2>/dev/null;
			rm $RUN_SAMPLE_DIR/$S.unaligned.bam
			mv $RUN_SAMPLE_DIR/$S.unaligned.tmp.bam $RUN_SAMPLE_DIR/$S.unaligned.bam
			#cat $RUN_SAMPLE_DIR/$S.unaligned.bam.validation2
			if ((0)); then
				if [ $(grep "^ERROR" $RUN_SAMPLE_DIR/$S.unaligned.bam.validation2 -c) -gt 0 ]; then
					echo "#[ERROR] Generated uBAM not fixed!";
					grep "^ERROR" $RUN_SAMPLE_DIR/$S.unaligned.bam.validation2;
					exit 0;
				else
					echo "#[INFO] Generated uBAM fixed";
					rm $RUN_SAMPLE_DIR/$S.unaligned.bam
					mv $RUN_SAMPLE_DIR/$S.unaligned.tmp.bam $RUN_SAMPLE_DIR/$S.unaligned.bam
				fi;
			fi;
		...
		echo "#[INFO] Validation uBAM"
		$JAVA $JAVA_FLAGS -jar $PICARD ValidateSamFile $PICARD_FLAGS $PICARD_VALIDATE_FLAGS $PICARD_VALIDATE_NAME_FLAGS VALIDATE_INDEX=true IGNORE_WARNINGS=true INDEX_VALIDATION_STRINGENCY=EXHAUSTIVE I=$RUN_SAMPLE_DIR/$S.unaligned.bam > $RUN_SAMPLE_DIR/$S.unaligned.bam.validation 2>/dev/null;
		#cat $RUN_SAMPLE_DIR/$S.unaligned.bam.validation
		if [ $(grep "^ERROR" $RUN_SAMPLE_DIR/$S.unaligned.bam.validation -c) -gt 0 ]; then
			echo "#[WARNING] Input file error. Generated uBAM file malformed! Try to fix it.";
			#grep "^ERROR" $RUN_SAMPLE_DIR/$S.unaligned.bam.validation;
			
			echo "#[INFO] Try to Fix Mate Information.";
			$SAMTOOLS sort -n $RUN_SAMPLE_DIR/$S.unaligned.bam -@ $THREADS | $SAMTOOLS fixmate - | $SAMTOOLS sort - -@ $THREADS > $RUN_SAMPLE_DIR/$S.unaligned.tmp.bam
			$JAVA $JAVA_FLAGS -jar $PICARD FixMateInformation $PICARD_FLAGS INPUT=$RUN_SAMPLE_DIR/$S.unaligned.tmp.bam ASSUME_SORTED=true VALIDATION_STRINGENCY=LENIENT 1>/dev/null 2>/dev/null;
			##exit 0;
			$JAVA $JAVA_FLAGS -jar $PICARD ValidateSamFile $PICARD_FLAGS $PICARD_VALIDATE_FLAGS $PICARD_VALIDATE_NAME_FLAGS VALIDATE_INDEX=true IGNORE_WARNINGS=true INDEX_VALIDATION_STRINGENCY=EXHAUSTIVE I=$RUN_SAMPLE_DIR/$S.unaligned.tmp.bam > $RUN_SAMPLE_DIR/$S.unaligned.bam.validation #2>/dev/null;
			cat $RUN_SAMPLE_DIR/$S.unaligned.bam.validation
			exit 0
			if [ $(grep "^ERROR" $RUN_SAMPLE_DIR/$S.unaligned.bam.validation -c) -gt 0 ]; then
				echo "#[ERROR] Generated uBAM Mate information not fixed!";
				grep "^ERROR" $RUN_SAMPLE_DIR/$S.unaligned.bam.validation;
				
				echo "#[INFO] Try to Read Groups Information.";
					$JAVA $JAVA_FLAGS -jar $PICARD AddOrReplaceReadGroups $PICARD_FLAGS $PICARD_ADDORREPLACEREADGROUPS_FLAGS $PICARD_ADDORREPLACEREADGROUPS_NAME_FLAGS I=$RUN_SAMPLE_DIR/$S.unaligned.bam O=$RUN_SAMPLE_DIR/$S.unaligned.tmp.bam 1>/dev/null 2>/dev/null
					$JAVA $JAVA_FLAGS -jar $PICARD ValidateSamFile $PICARD_FLAGS $PICARD_VALIDATE_FLAGS $PICARD_VALIDATE_NAME_FLAGS VALIDATE_INDEX=true IGNORE_WARNINGS=true INDEX_VALIDATION_STRINGENCY=EXHAUSTIVE I=$RUN_SAMPLE_DIR/$S.unaligned.tmp.bam > $RUN_SAMPLE_DIR/$S.unaligned.bam.validation 2>/dev/null;
					if [ $(grep "^ERROR" $RUN_SAMPLE_DIR/$S.unaligned.bam.validation -c) -gt 0 ]; then
						echo "#[ERROR] Generated uBAM Read Groups Information not fixed!";
						grep "^ERROR" $RUN_SAMPLE_DIR/$S.unaligned.bam.validation;
						exit 0;
					else
						echo "#[INFO] Generated uBAM Read Groups Information fixed";
						rm $RUN_SAMPLE_DIR/$S.unaligned.bam
						mv $RUN_SAMPLE_DIR/$S.unaligned.tmp.bam $RUN_SAMPLE_DIR/$S.unaligned.bam
					fi;
				...
				echo "#[INFO] Generated uBAM Mate Information fixed";
				#rm $RUN_SAMPLE_DIR/$S.unaligned.bam
				#mv $RUN_SAMPLE_DIR/$S.unaligned.tmp.bam $RUN_SAMPLE_DIR/$S.unaligned.bam
			fi;
			...
			if ((0)); then
				...
				then
					...
					echo "#[ERROR] Generated uBAM not able to be fixed!";
					exit 0;
				fi;
			fi;
	...
	make -k -j $THREADS -e NGSEnv=$NGS_FOLDER ENV=$ENV PARAM=$MAKEFILE_ANALYSIS_RUN $PARAMETERS $THREAD_PARAMETERS JAVA_MEMORY=$JAVA_MEMORY SNAPSHOT=0 VALIDATION=1 INPUT=$INPUT OUTDIR=$OUTPUT RELEASE=$RELEASE_RUN -f $NGS_SCRIPTS/NGSWorkflow.mk 1>>$LOGFILE_RES_RUN 2>>$LOGFILE_RES_RUN
	...
	#make -k -j $THREADS -e NGSEnv=$NGS_FOLDER ENV=$ENV PARAM=$MAKEFILE_ANALYSIS_RUN $PARAMETERS $THREAD_PARAMETERS JAVA_MEMORY=$JAVA_MEMORY SNAPSHOT=0 VALIDATION=1 INPUT=$INPUT OUTDIR=$OUTPUT RELEASE=$RELEASE_RUN FINAL_REPORT=$FINAL_REPORT_RUN ANALYSIS_REF=$ANALYSIS_REF FINAL_REPORT_FULL=$FINAL_REPORT_FULL_RUN -f $NGS_SCRIPTS/NGSWorkflow.mk  1>>$LOGFILE_RES_RUN_REPORT 2>>$LOGFILE_RES_RUN_REPORT
	make -k -j $THREADS -e NGSEnv=$NGS_FOLDER ENV=$ENV PARAM=$MAKEFILE_ANALYSIS_RUN $PARAMETERS $THREAD_PARAMETERS JAVA_MEMORY=$JAVA_MEMORY SNAPSHOT=0 VALIDATION=1 INPUT=$INPUT OUTDIR=$OUTPUT RELEASE=$RELEASE_RUN FINAL_REPORT=$FINAL_REPORT_RUN ANALYSIS_REF=$ANALYSIS_REF -f $NGS_SCRIPTS/NGSWorkflow.mk  1>>$LOGFILE_RES_RUN_REPORT 2>>$LOGFILE_RES_RUN_REPORT
					...
					if [ $RESULTS_SUBFOLDER_DATA != "" ]; then
						for ROOT_FILE_PATTERN in $RESULTS_SUBFOLDER_ROOT_FILE_PATTERNS; do
							...
							then
								...
								#while ! $COMMAND_COPY "$OUTPUT/$RUU/$S/$ROOT_FILE_PATTERN_VAR" "$RESULTS_FOLDER_COPY_FOLDER/$RUU/$S" 1>>$LOGFILE_RES_RUN_REPORT 2>>$LOGFILE_RES_RUN_REPORT ; do : ; done;
								$COMMAND_COPY "$OUTPUT/$RUU/$S/$ROOT_FILE_PATTERN_VAR" "$RESULTS_FOLDER_COPY_FOLDER/$RUU/$S" 1>>$LOGFILE_RES_RUN_REPORT 2>>$LOGFILE_RES_RUN_REPORT
							else
								#while ! $COMMAND_COPY $OUTPUT/$RUU/$S/$ROOT_FILE_PATTERN $RESULTS_FOLDER_COPY_FOLDER/$RUU/$S 1>>$LOGFILE_RES_RUN_REPORT 2>>$LOGFILE_RES_RUN_REPORT ; do : ; done;
								$COMMAND_COPY $OUTPUT/$RUU/$S/$ROOT_FILE_PATTERN $RESULTS_FOLDER_COPY_FOLDER/$RUU/$S 1>>$LOGFILE_RES_RUN_REPORT 2>>$LOGFILE_RES_RUN_REPORT
							fi
						done;
					fi;
				done;
