...
VCF_INPUT=$1 	# EQA_NEQAS.bwamem.gatkHC.vap.vcf ANNOTATED with HOWARD
...
cat $VCF_EXONIC | $VCFTOOLS/vcf-annotate -r "$FieldsToRevome" | grep -v "^##source_" | sed 's/EQA_NEQAS/Lab_0036/g'  > $VCF_SIMPLIFIED
...
echo "# NB variants input : "$($SAMTOOLS view -c $VCF_INPUT)
echo "# NB variants output: "$($SAMTOOLS view -c $VCF_SIMPLIFIED)
echo "$SAMTOOLS view -c $VCF_INPUT"
# Validation
echo "# VCF Validation:"
