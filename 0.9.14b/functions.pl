...
sub read_ini {
#$_[0]: The ini file
#$_[1]: case of the headers/variables, either NULL, "lower" or "upper"
	#parameters
	$project_infos_file=$_[0];
	$case=lc($_[1]);
	#clear variable
	#undef $project_infos;
	my %project_infos;
	
	#foreach $key (keys %project_infos) {
		#delete $project_infos{$key};
	#};#forwach
        #initialisation
	$header="";
	if (-e $project_infos_file) { #if file exist
		#open the file
		open(FILE_PROJECT_INFOS, $project_infos_file) || die "Problem to open the file: $!";
		#read the file		
		while(<FILE_PROJECT_INFOS>) {
			...
			if (/(\S+)\s*[=:\t]\s*(.*?)\s*$/) {
				...
				$project_infos{$header}{$var} = $val;
			};#if
		};#while
		close(FILE_PROJECT_INFOS);
		return %project_infos;
	} else {
		print "File '$project_infos_file' DOES NOT exist\n";
		exit 1;
	};#if
}
...
# return $annotation_output (hash ref), $annotations (hash ref)
	...
	while(<VCF>) {
		...
		# VARIANT
			...
			# INFOS col
			$infos=$line_content_split[7];
			@infos_split=split(";",$infos);
			foreach $info (@infos_split) {
				@info_split=split("=",$info,2);
				$info_name=$info_split[0];
				$info_value=$info_split[1];
				$annotation_output{$chr}{$pos}{$ref}{$alt}{$info_name}=$info_value;
			};#foreach
